...
class collector_api():
    ...
    def realtime_daily_buy_list_check(self):
        if self.open_api.sf.is_date_exist(self.open_api.today):
            ...
            # 첫 번째 파라미터는 오늘일자, 두 번째 파라미터는 어제 일자. 세번째는 date_rows 인덱스
            self.open_api.sf.db_to_realtime_daily_buy_list(self.open_api.sf.date_rows[-1][0], self.open_api.sf.date_rows[-2][0], len(self.open_api.sf.date_rows) - 1)
